...
export const CardBody: React.FC<CardBodyProps> = ({ quotation }) => {
  const formatValue = (quotation: FinancialData): string => {
    if (quotation.type !== QUOTATION_TYPE.STOCK) {
  ...
  return (
    <div className="flex-grow space-y-2">
      ...
      {(quotation.type === QUOTATION_TYPE.CURRENCY || quotation.type === QUOTATION_TYPE.BITCOIN) && (
        <CardInfoItem 
          label="Venda"
      ...
      {quotation.type === QUOTATION_TYPE.STOCK && quotation.location && (
        <CardInfoItem 
          label="Local"
